...
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        TotalElectoralCollegeTies test0 = new TotalElectoralCollegeTies(electoralVotes2024);
        System.out.println(test0.run());

        MinStateElectoralCollegeTie test1 = new MinStateElectoralCollegeTie(electoralVotes2024);
        ArrayList<Integer> test2 = test1.run();
        System.out.println(test2);
        int total = 0;
        for (int i = 0; i < test2.size(); i++) {
            total+= electoralVotes2024[test2.get(i)];
            System.out.print(states[test2.get(i)] + " ");
        }
        System.out.println("\n" + total);
    }
}