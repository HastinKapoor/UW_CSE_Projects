...
(define (eval-under-env e env)
  (cond [(var? e)
	 ...
	 (let ([cl (eval-under-env (call-funexp e) env)]
	   ...
	   (if (closure? cl)
               (let ([env1 (append (list (cons (fun-formal (closure-fun cl)) act)
                                         (if (fun-nameopt (closure-fun cl))
                                             (cons (fun-nameopt (closure-fun cl)) cl)
                                             '()))(closure-env cl))])
	       (eval-under-env (fun-body (closure-fun cl)) env1))
...
(define (mlet* bs e2)
  (if (null? bs)
      e2
      (let ([x (car bs)])
        (mlet (car x) (cdr x) (mlet* (cdr bs) e2)))))
      ...
      (fun "mupl-filter-lst" "lst"
	   (ifnz (ismunit (var "lst"))
		 ...
		 (ifnz (call (var "fn") (first (var "lst")))
                       (apair (first (var "lst")) (call (var "mupl-filter-lst") (second (var "lst"))))
                       (call (var "mupl-filter-lst") (second (var "lst"))))))))

(define mupl-all-gt
  (mlet "filter" mupl-filter
        (fun "mupl-all-gt-int" "i"
             (fun "mupl-all-gt-lst" "lst"
                  (call (call (var "filter") (fun null "x" (isgreater (var "x") (var"i")))) (var "lst"))))))
